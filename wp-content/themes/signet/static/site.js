(() => {
  ...
  function buttonAnimation() {
    ...
    if (buttons.length === 0)
      return;
    document.fonts.ready.then(() => buttonCallback(buttons));
  }
  function buttonCallback(buttons) {
    buttons.forEach((button) => {
      const buttonIcon = button.querySelector(".btn-icon");
      const buttonTextWrapper = button.querySelector(".btn-text");
      if (!buttonTextWrapper)
        return;
      const buttonText = buttonTextWrapper.querySelector("span");
      if (!buttonText)
        return;
      const textClone = buttonText.cloneNode(true);
      gsapWithCSS.set(textClone, {
        position: "absolute",
        left: 0,
        opacity: 0.75,
        top: "100%",
      ...
      buttonTextWrapper.appendChild(textClone);
      const splitText_1 = SplitText.create(buttonText, {});
      const splitText_2 = SplitText.create(textClone, {});
  ...
  var Observer = /* @__PURE__ */ function() {
    ...
    _proto.init = function init4(vars) {
      _coreInitted3 || _initCore3(gsap3) || console.warn("Please gsap.registerPlugin(Observer)");
  ...
  function toggleMenu() {
    ...
    const elementsStagger = document.querySelectorAll("[data-stagger-items]");
    let isOpen = false;
  ...
  function talentScroll() {
    const talentPopup = document.querySelector("[talent-popup]");
    if (!talentPopup)
      return;
    const content = talentPopup.querySelector(".container");
  ...
  var ParallaxEffect = class {
    ...
    init() {
      gsapWithCSS.registerPlugin(ScrollTrigger2);
      this.elements = gsapWithCSS.utils.toArray(this.options.selector);
    ...
    createParallaxEffects() {
      this.elements.forEach((element, index) => {
        const img = element.querySelector(this.options.imageSelector);
        if (!img) {
          console.warn(`No image found in parallax element ${index + 1}`);
          return;
        }
        const speed = element.getAttribute("parallax-effect") ? element.getAttribute("parallax-effect") : this.options.speed;
        gsapWithCSS.set(img, {
        ...
        const tl = gsapWithCSS.timeline({
          scrollTrigger: {
            ...
            refreshPriority: this.options.refreshPriority,
            onUpdate: (self) => {
              const progress = self.progress;
            }
          }
        });
        const moveDistance = window.innerHeight * parseFloat(speed);
        tl.fromTo(
          img,
          {
            y: -moveDistance
    ...
    destroy() {
      this.elements.forEach((element) => {
        const img = element.querySelector(this.options.imageSelector);
        if (img && img.parallaxTimeline) {
          img.parallaxTimeline.kill();
          gsapWithCSS.set(img, { clearProps: "all" });
        }
      });
      ScrollTrigger2.getAll().forEach((trigger) => {
        if (trigger.vars.trigger && this.elements.includes(trigger.vars.trigger)) {
          trigger.kill();
        }
      });
      console.log("destroy");
  ...
  // js/components/talent-popup.js
  gsapWithCSS.registerPlugin(ScrollTrigger2);
  function initTalentPopup() {
    const talentButtons = document.querySelectorAll("[talent-button]");
    const talentPopup = document.querySelector("[talent-popup]");
    if (!talentPopup)
      return;
    const closeButton = document.querySelector("[data-close-talent]");
    const popupTitle = talentPopup.querySelector("[talent-title]");
    const popupLongDescription = talentPopup.querySelector("[long-description]");
    const popupCollaborations = talentPopup.querySelector(
      "[recent-collaborations]"
    );
    const popupLocation = talentPopup.querySelector("[location]");
    const popupGallery = talentPopup.querySelector("[talent-gallery]");
    ...
    function getSlugFromTalentText(talentText) {
      const slugDiv = talentText.querySelectorAll(".hidden")[4];
      return slugDiv ? slugDiv.textContent.trim() : "";
    }
    function scrollToTalentSection(slug, instant = false) {
      const width = window.innerWidth;
      let talentSection;
      if (width < 1024) {
        talentSection = document.querySelector(`#${slug}.mobile`);
      } else {
        talentSection = document.querySelector(`#${slug}`);
      }
      if (talentSection && Lenis2) {
        Lenis2.scrollTo(talentSection, {
          immediate: instant
        });
      }
    }
    function populatePopup(talentIndex) {
      const talentText = document.querySelector(`[talent-text="${talentIndex}"]`);
      if (!talentText)
        return;
      const title = talentText.querySelector("h2").textContent.trim();
      popupTitle.textContent = title;
      const longDescDiv = talentText.querySelectorAll(".hidden")[0];
      if (longDescDiv) {
        popupLongDescription.innerHTML = longDescDiv.innerHTML;
      }
      const collaborationsDiv = talentText.querySelectorAll(".hidden")[2];
      popupCollaborations.innerHTML = "";
      if (collaborationsDiv) {
        const collabPs = collaborationsDiv.querySelectorAll("p");
        collabPs.forEach((p, index) => {
          const span = document.createElement("span");
          span.textContent = p.textContent.trim();
          span.className = "text-sm";
          popupCollaborations.appendChild(span);
          if (index < collabPs.length - 1) {
            const separator = document.createElement("span");
            separator.textContent = "\u2022";
            popupCollaborations.appendChild(separator);
          }
        });
      }
      const locationDiv = talentText.querySelectorAll(".hidden")[3];
      if (locationDiv) {
        popupLocation.textContent = locationDiv.textContent.trim();
      }
      const galleryDiv = talentText.querySelectorAll(".hidden")[1];
      popupGallery.innerHTML = "";
      if (galleryDiv) {
        const galleryImages = galleryDiv.querySelectorAll("[gal-images-wrapper]");
        galleryImages.forEach((img) => {
          const clonedImg = img.cloneNode(true);
          popupGallery.appendChild(clonedImg);
        });
        parallax = new ParallaxEffect({
          selector: "[talent-gallery] [gal-images]",
          scroller: "[talent-popup]"
        });
        tl.from(
          "[talent-popup] [gal-images-wrapper]",
          {
            y: 48,
            duration: 0.5,
            opacity: 0,
    ...
    function getTalentIndexFromSlug(slug) {
      const buttons = Array.from(talentButtons);
      return buttons.find((button) => {
        const index = button.getAttribute("talent-button");
        const talentText = document.querySelector(`[talent-text="${index}"]`);
        if (talentText) {
          const talentSlug = getSlugFromTalentText(talentText);
          return talentSlug === slug;
        }
        return false;
      })?.getAttribute("talent-button");
    }
    function openPopup(talentIndex, updateURL = true, shouldScroll = false) {
      populatePopup(talentIndex);
      const talentText = document.querySelector(`[talent-text="${talentIndex}"]`);
      const slug = getSlugFromTalentText(talentText);
      if (shouldScroll) {
        scrollToTalentSection(slug, shouldScroll);
        tl.play().then(() => {
          Lenis2.stop();
        });
      } else {
        scrollToTalentSection(slug, false);
        tl.play().then(() => {
          Lenis2.stop();
        });
      }
      if (updateURL) {
        history.pushState({ talent: slug }, "", `?talent=${slug}`);
      }
    }
    function closePopup(updateURL = true) {
      tl.reverse().then(() => {
        Lenis2.start();
        parallax?.destroy();
      });
    ...
    window.addEventListener("popstate", (e) => {
      ...
      const talent2 = params2.get("talent");
      if (talent2) {
        const talentIndex = getTalentIndexFromSlug(talent2);
        if (talentIndex) {
          openPopup(talentIndex, false);
        }
      } else {
        closePopup(false);
      }
    ...
    if (talent) {
      const talentIndex = getTalentIndexFromSlug(talent);
      if (talentIndex) {
        openPopup(talentIndex, false, true);
      }
    }
    talentButtons.forEach((button) => {
      button.addEventListener("click", (e) => {
        e.preventDefault();
        const talentIndex = button.getAttribute("talent-button");
        openPopup(talentIndex);
      });
    });
    if (closeButton) {
    ...
    talentPopup.addEventListener("click", (e) => {
      if (e.target === talentPopup) {
        closePopup();
      }
    });
  ...
  // js/components/talent-swiper.js
  gsapWithCSS.registerPlugin(ScrollTrigger2);
  ...
  // js/components/talent-tease.js
  gsapWithCSS.registerPlugin(ScrollTrigger2);
  function talentTeaseAnimations() {
    const talentTeaseElements = document.querySelectorAll("[talent-tease]");
    if (!talentTeaseElements.length > 0)
      return;
    scrollAnimations(talentTeaseElements);
    hoverAnimations(talentTeaseElements);
    handleWindowResize();
  }
  function scrollAnimations(talentTeaseElements) {
    talentTeaseElements.forEach((element) => {
      const talentInfo = element.querySelector("[talent-info]");
      if (!talentInfo)
        return;
      const { height: containerHeight } = element.getBoundingClientRect();
      const { height: infoHeight } = talentInfo.getBoundingClientRect();
      const innerItems = talentInfo.querySelectorAll(":scope > *");
      if (!innerItems.length)
        return;
      gsapWithCSS.set(innerItems, {
        opacity: 0,
        yPercent: 100
      });
      let tl = gsapWithCSS.timeline({
      ...
      ScrollTrigger2.create({
        trigger: talentInfo,
        start: `bottom bottom`,
        end: `bottom bottom-=${containerHeight - infoHeight}px`,
        pin: talentInfo,
        scrub: true,
        markers: false,
        onEnter: () => {
  ...
  function hoverAnimations(talentTeaseElements) {
    talentTeaseElements.forEach((element) => {
      const talentInfo = element.querySelector("[talent-info]");
      if (!talentInfo)
        return;
      const icon = talentInfo.querySelector(".btn-icon");
      if (!icon)
        return;
      ...
      const titleContainer = talentInfo.querySelector("h3");
      if (!titleContainer)
        return;
      gsapWithCSS.set(titleContainer, {
        x: -24
      });
      let tl = gsapWithCSS.timeline({
        paused: true,
  ...
  function handleWindowResize() {
    window.addEventListener("resize", () => {
      ScrollTrigger2.getAll().forEach((trigger) => {
        if (trigger.trigger && trigger.trigger.hasAttribute("talent-info")) {
          trigger.kill();
        }
      });
      scrollAnimations(document.querySelectorAll("[talent-tease]"));
    });
  }
  ...
  function initHeaderOnScroll() {
    ...
    if (!header)
      return;
    const headingText = header.querySelector(".header-text");
  ...
  function updateTimes() {
    ...
    const melElements = document.querySelectorAll(".mel-time");
    const lonElements = document.querySelectorAll(".long-time");
    const nycElements = document.querySelectorAll(".nyc-time");
    if (melElements.length > 0)
      melElements.forEach((el) => el.textContent = melbourneTime);
    if (lonElements.length > 0)
      lonElements.forEach((el) => el.textContent = londonTime);
    if (nycElements.length > 0)
      nycElements.forEach((el) => el.textContent = newYorkTime);
  }
