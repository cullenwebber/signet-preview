(() => {
  ...
  // js/components/image-swiper.js
  function imageSwiper() {
    const swiper = document.querySelector(".image-swiper");
    if (!swiper)
      return;
    new Swiper(swiper, {
      loop: true,
  ...
  function talentBanner() {
    ...
    talentMenuItems.forEach((item, index) => {
      item.addEventListener("mouseover", () => {
        gsapWithCSS.killTweensOf(talentImages);
        talentImages.forEach((image, i) => {
          if (i !== index) {
            gsapWithCSS.to(image, { opacity: 0, duration: 0.5, ease: "power2.out" });
          }
        });
  ...
  function showEmail() {
    const emailButton = document.querySelector("[show-email]");
    if (!emailButton)
      return;
    const email = document.querySelector(".email-link");
    emailButton.addEventListener("click", () => {
      email.classList.toggle("-translate-y-full");
    });
  }
  ...
  function updateTimes() {
    const now2 = new Date();
    const sydneyTime = now2.toLocaleTimeString("en-US", {
    ...
    const londonTime = now2.toLocaleTimeString("en-US", {
    ...
    const newYorkTime = now2.toLocaleTimeString("en-US", {
    ...
    if (melElements.length > 0)
      melElements.forEach((el) => el.textContent = sydneyTime);
